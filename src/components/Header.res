    ...
    <>
        ...
        {isNavOpen?
            <div className="flex flex-col justify-between h-[calc(100vh-2.5rem)] pt-12 pb-9 animate-slide-in-left">
                <NavMenu onClick={_=>setIsNavOpen(_=>false)} className="flex flex-col gap-8" textSize=ExtraLarge/>
                <div className="flex justify-center gap-2">
                {
                    CustomUtils.maxNelems(Data.profiles,3)->Js.Array2.mapi((item,i)=><SocialMediaItem profile=item size=Large key=Belt.Int.toString(i) />)->React.array
                }
                </div>
            </div>
        :React.null}
